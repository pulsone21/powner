...
func (s Service) CreateTeam(name, description string, skills *[]entities.Skill, members *[]entities.Member) (*entities.Team, error) {
	// INFO: validate inputs

	t, err := s.TeamRepo.Create(*entities.NewTeam(name, description))
...
func (s Service) UpdateTeam(id, name, description string, skills *[]entities.Skill, members *[]entities.Member) (*entities.Team, error) {
	var validationErrors errx.ErrorMap
	fid, err := strconv.Atoi(id)
	if err != nil {
		validationErrors.Set("id", err)
	...
	nT, change := oldT.HasChanges(name, description, skills, members)
	if change {
